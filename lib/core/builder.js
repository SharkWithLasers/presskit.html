...
async function build (dataFilePath, presskit, {
  ...
  const pageFolder = getPageFolder(buildFolder, dataFilePath)
...
function getPageFolder (buildFolder, dataFilePath) {
  // Logic:
  ...
  // MJK: just place it in build folder...we are doing a single <company>...that is a product actually lol
  /*
  if (pageType !== 'company') {
  ...
  sfs.findAllFiles(source)
    .forEach(filePathWithSource => {
      ...
      if (f.toLowerCase().startsWith('header')) {
        images.header = f
      } else if (f.toLowerCase().startsWith('logodark')) {
        // won't double count if we start with the more stringent one first
        // will also make sure order is correct..
        images.logos.push({ fileName: f, logoDark: true});
      } else if(f.toLowerCase().startsWith('logo')) {
        images.logos.push({ fileName: f, logoDark: false})
      } 
      else {
...
function exportArchives (images, source, target) {
  ...
  createArchive('logo.zip', target, source, images.logos.map(x => x.fileName))
}
